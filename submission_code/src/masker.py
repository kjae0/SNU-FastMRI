...
class MaskFunc:
    ...
    def __init__(
        ...
        self._center_fractions = center_fractions
        ...
        if self.n_steps:
            ...
            self.center_fractions = self._center_fractions
            self.accelerations = self._accelerations
    ...
    def __call__(
        ...
        with temp_seed(self.rng, seed):
            center_mask, accel_mask, num_low_frequencies, acceleration = self.sample_mask(
                shape, offset
            )

        # combine masks together
        return torch.max(center_mask, accel_mask), num_low_frequencies, acceleration
    
    def step(self):
        assert self.n_steps
        self.n_steps_left -= 1
        
        n_accs = len(self._accelerations)
        ratio = min(self.start_ratio + (1 - self.start_ratio) * (self.n_steps - self.n_steps_left) / self.n_steps, 1)
        idx = int(n_accs * ratio)
        
        self.center_fractions = self._center_fractions[:idx]
        self.accelerations = self._accelerations[:idx]
        
        print(f"center_fractions: {self.center_fractions}, accelerations: {self.accelerations}")
        print(f"n_steps_left: {self.n_steps_left}")
        print(f"ratio: {ratio}")
        print(f"idx: {idx}")
...
class MagicMaskFunc(MaskFunc):
    ...
    def calculate_acceleration_mask(
        ...
        """
        if offset is None:
            print("mmf calculate acceleration mask", acceleration)
