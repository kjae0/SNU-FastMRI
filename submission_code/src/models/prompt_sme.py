...
class SensitivityModel(nn.Module):
    ...
    def divide_root_sum_of_squares(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # x = torch.where(
        #     fastmri.rss_complex(x, dim=2).unsqueeze(-1).unsqueeze(2) == 0,
        #     torch.tensor([0.0], dtype=x.dtype).to(x.device),
        #     x / fastmri.rss_complex(x, dim=2).unsqueeze(-1).unsqueeze(2),
        # )
        x = x / fastmri.rss_complex(x, dim=2).unsqueeze(-1).unsqueeze(2)
        # return torch.where(
        #     fastmri.rss_complex(x, dim=1) == 0,
        #     torch.tensor([0.0], dtype=x.dtype).to(x.device),
        #     x / fastmri.rss_complex(x, dim=1).unsqueeze(-1).unsqueeze(1),
        # )
