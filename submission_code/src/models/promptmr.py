...
class PromptMR(nn.Module):
    ...
    def __init__(
        ...
        self.train_sens_net = train_sens_net
        
        if self.train_sens_net == False:
            for _ in range(10):
                print("WARNING!!! Sensitivity network is not trainable.")
    ...
    def forward(
        ...
        output_image = fastmri.rss(fastmri.complex_abs(fastmri.ifft2c(kspace_pred)), dim=1)
        height = output_image.shape[-2]
        width = output_image.shape[-1]
        output_image = output_image[..., (height - 384) // 2 : 384 + (height - 384) // 2, (width - 384) // 2 : 384 + (width - 384) // 2]

        return {'image': output_image, 'kspace': kspace_pred}
